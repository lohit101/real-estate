...
export default function Navbar() {
  ...
  return (
    <nav className={`fixed top-0 left-0 z-40 w-full flex flex-row items-center justify-between transition-all duration-1000 bg-linear-to-b from-black/50 to-transparent ${isScrolled ? "py-4 px-3" : "py-4 px-5"}`}>
      <div className="fixed top-0 left-0 z-30 flex h-28 w-full bg-transparent backdrop-blur-xs [mask-image:linear-gradient(to_bottom,white,transparent)] transition-all duration-1000"></div>
      <div className="flex z-50 w-full flex-row items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          ...
          {/* MOBILE MENU */}
          <div className={`sm:hidden fixed top-0 left-0 w-screen h-screen flex bg-red-500/50 backdrop-blur-xs z-50 ${menuOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'} transition-all duration-300`}>
