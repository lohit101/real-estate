...
export default function Navbar() {
  ...
  const solidNav = isScrolled || (pathname !== "/" && pathname !== "/about");
  ...
  return (
    <nav data-solid={solidNav} className={`site-surface site-nav fixed top-0 left-0 z-40 w-full flex flex-row items-center justify-between transition-all duration-1000 bg-linear-to-b from-black/50 to-transparent ${isScrolled ? "py-4 px-3" : "py-4 px-5"}`}>
      <div className="fixed top-0 left-0 z-30 flex h-28 w-full bg-transparent backdrop-blur-xs mask-[linear-gradient(to_bottom,white,transparent)] transition-all duration-1000"></div>
      <div className="flex z-50 w-full flex-row items-center justify-between">
        <Link href="/" onClick={() => setMenuOpen(false)}>
          <div className="flex flex-row items-center gap-2 w-max opacity-100">
            ...
            <img src='https://web.archive.org/web/20211205032847im_/http://www.1o1realtor.com/static/img/LogoWhite.png' alt="Logo" className={`h-5 sm:h-10 ${solidNav ? 'invert' : ''} drop-shadow-md transition-all duration-500`} />
            {/* <div className="flex rounded-md w-12 h-12 aspect-square bg-zinc-900"></div> */}

            <div className={`hidden sm:flex flex-col ${isScrolled ? 'w-[0%]' : 'w-full'} overflow-hidden transition-all duration-1000 text-white`}>
        ...
        <div className="flex items-center gap-4">
          {links.map((link) => (
            <div key={link.name} className="group hidden sm:flex flex-col">
              <Link href={link.href} className={`text-sm drop-shadow-lg font-medium ${solidNav ? 'text-black' : 'text-white'}`}>
                {link.name}
              </Link>
              <div className={`flex h-px w-0 group-hover:w-full ${solidNav ? 'bg-black' : 'bg-white'} transition-all duration-500`}></div>
            </div>
          ))}
          ...
          {/* MOBILE MENU */}
          <div className={`sm:hidden fixed top-0 left-0 w-screen h-screen flex bg-black/50 backdrop-blur-xs z-50 ${menuOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'} transition-all duration-300`}>
              ...
              <div className="flex flex-col gap-2">
                {links.map((link, key) => (
                  <div key={key} className="group flex flex-col">
                    ...
                    <div className={`flex h-px w-0 group-hover:w-full bg-black transition-all duration-500`}></div>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="flex sm:hidden flex-col gap-3" onClick={() => setMenuOpen(true)}>
            <div className={`flex h-[2px] w-10 rounded-full ${solidNav ? 'bg-black' : 'bg-white'}`}></div>
            <div className={`flex h-[2px] w-8 rounded-full ${solidNav ? 'bg-black' : 'bg-white'}`}></div>
          </div>
